...
public class TCTraffic : MonoBehaviour
{
        ...
        if (!locked)
        {
            if (VehicleCount < MaxVehicleCount)
            {
                for (int k0 = 0; k0 < SpawnPoints.Length; k0++)
                {
                    ...
                    if (obj.transform.gameObject.GetComponent<TCVehicle>())
                    {
                        obj.transform.gameObject.GetComponent<TCVehicle>()._TCTrafficLinked = transform.gameObject.GetComponent<TCTraffic>();
                        obj.transform.gameObject.GetComponent<TCVehicle>().CarSpeed += CarSpeedVariations[k0];
                    }
                    else
                    {
